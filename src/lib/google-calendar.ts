...
function eventUrl(input: EventInput, eventId?: string) {
  ...
  const url = new URL(
    `${CALENDAR_API}/calendars/${encodeURIComponent(calendarId)}/events${eventId ? `/${encodeURIComponent(eventId)}` : ""}`
  );
...
export async function updateCalendarEvent(
  input: EventInput & { eventId: string }
...
export async function deleteCalendarEvent(input: {
  ...
  const url = new URL(
    `${CALENDAR_API}/calendars/${encodeURIComponent(input.calendarId ?? "primary")}/events/${encodeURIComponent(input.eventId)}`
  );
