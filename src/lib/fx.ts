...
export async function getNgnPerUsd(): Promise<FxQuote> {
  if (!env.OPEN_EXCHANGE_RATES_APP_ID) return { available: false, fetchedAt: new Date().toISOString(), source: "unconfigured" };
  try {
    const response = await fetch(`https://openexchangerates.org/api/latest.json?app_id=${encodeURIComponent(env.OPEN_EXCHANGE_RATES_APP_ID)}&symbols=NGN`, { next: { revalidate: 900 } });
    ...
    return { available: true, ngnPerUsd: payload.rates.NGN, fetchedAt: new Date((payload.timestamp ?? Date.now()/1000)*1000).toISOString(), source: "Open Exchange Rates" };
  } catch {
