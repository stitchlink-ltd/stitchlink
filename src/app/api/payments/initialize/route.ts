...
export async function POST(request:Request){
  ...
  if(order.deposit_paid_at||order.balance_paid_at)return Response.json({error:"This order has already been paid."},{status:409});
  const amountKobo=order.tailoring_subtotal_kobo+order.delivery_kobo;const reference=`STL-FUL-${randomUUID()}`;
  const {data:pending}=await supabase.from("payments").select("reference,authorization_url,amount_kobo").eq("order_id",order.id).eq("installment","full").eq("status","pending").maybeSingle();
  ...
  const {error:insertError}=await supabase.from("payments").insert({order_id:order.id,customer_id:user.id,reference,installment:"full",amount_kobo:amountKobo,currency:"NGN",status:"pending"});
  if(insertError)return Response.json({error:"Unable to reserve payment reference"},{status:500});
  try{const data=await initializePaystackTransaction({email:user.email!,amountKobo,reference,callbackUrl:`${env.NEXT_PUBLIC_SITE_URL}/customer/payments?reference=${encodeURIComponent(reference)}`,metadata:{order_id:order.id,installment:"full"}});await supabase.from("payments").update({authorization_url:data.authorization_url,initialized_at:new Date().toISOString()}).eq("reference",reference);return Response.json({authorizationUrl:data.authorization_url,reference,amountKobo,currency:"NGN"});}catch{await supabase.from("payments").update({status:"failed",initialization_error:"Paystack initialization failed"}).eq("reference",reference);return Response.json({error:"Paystack initialization failed"},{status:502});}
}
