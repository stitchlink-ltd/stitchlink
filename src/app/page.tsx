    ...
    <PublicShell>
      <section className="relative min-h-[720px] overflow-hidden bg-ink text-white lg:min-h-[760px]">
        ...
        <div className="container-shell relative flex min-h-[720px] items-center pb-16 pt-24 lg:min-h-[760px]">
          <div className="max-w-[660px]">
            <p className="eyebrow mb-5 flex items-center gap-2 text-[#e2c18f]">
              <span className="h-px w-8 bg-[#e2c18f]" /> Bespoke, without borders
            </p>
            <h1 className="font-display text-[clamp(3.5rem,7vw,6.8rem)] font-medium leading-[.9] tracking-[-.055em]">
              Made for you.
              <br />
              <em className="font-normal text-[#e3caa6]">Made to matter.</em>
            </h1>
            <p className="mt-7 max-w-xl text-base leading-7 text-white/70 sm:text-lg">
              Meet Nigeria&apos;s most trusted tailors. Share your vision, agree every detail, and
              follow the making of a piece that is unmistakably yours.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <ButtonLink href="/tailors" className="bg-[#f4eee5] text-ink hover:bg-white" arrow>
                Find your tailor
              </ButtonLink>
              <ButtonLink
                href="/join"
            ...
            <div className="mt-10 flex flex-wrap gap-x-7 gap-y-3 text-xs text-white/65">
              {["Manually verified", "Protected payments", "Progress you can see"].map((item) => (
                <span key={item} className="flex items-center gap-1.5">
                  <Check size={14} className="text-[#e2c18f]" /> {item}
                </span>
              ))}
            </div>
          </div>
        </div>
        <div className="absolute bottom-0 right-0 hidden w-[38%] border-l rounded-l-lg border-t border-white/20 bg-black/25 p-5 backdrop-blur-lg lg:block">
          <div className="flex items-center justify-between">
            <div>
              <p className="eyebrow text-[#e2c18f]">Featured atelier</p>
              <p className="mt-1 font-display text-xl">Adaobi Atelier</p>
            </div>
            <Link
              href="/tailors/adaobi-atelier"
              className="grid size-11 place-items-center rounded-full border border-white/25 hover:bg-white hover:text-ink"
              aria-label="View Adaobi Atelier"
            >
              <ArrowRight size={18} />
            </Link>
          </div>
      ...
      <section className="border-b border-line bg-paper">
        <div className="container-shell -translate-y-4 rounded-lg border border-line bg-paper p-3 soft-shadow">
          <form action="/tailors" className="grid gap-2 md:grid-cols-[1fr_1fr_1fr_auto]">
            ...
            <button className="rounded-lg bg-wine px-7 py-4 text-sm font-semibold text-white transition hover:bg-wine-dark">
              Search tailors
            </button>
          </form>
        </div>
        <div className="container-shell grid grid-cols-2 gap-6 pb-14 pt-4 text-center md:grid-cols-4">
          {[
            ["120+", "verified ateliers"],
            ["2,400+", "pieces completed"],
      ...
      <section className="container-shell py-24">
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          <div>
            <p className="eyebrow">Curated for craft</p>
            <h2 className="mt-3 max-w-xl font-display text-4xl leading-tight sm:text-5xl">
              Meet tailors with a reputation worth wearing.
            </h2>
          </div>
          <ButtonLink href="/tailors" variant="secondary" arrow>
            Explore all tailors
          </ButtonLink>
        </div>
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {tailors.map((tailor, index) => (
            <TailorCard key={tailor.id} tailor={tailor} priority={index < 2} />
          ))}
        </div>
      </section>

      <section id="how-it-works" className="paper-grid border-y border-line bg-background py-24">
        <div className="container-shell">
          <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr]">
            <div className="lg:sticky lg:top-28 lg:self-start">
              <p className="eyebrow text-wine">From idea to arrival</p>
              <h2 className="mt-3 font-display text-5xl leading-[1.03]">
                One thoughtful process. No anxious guessing.
              </h2>
              <p className="mt-5 max-w-md leading-7 text-muted">
                Every decision, measurement, payment and update lives in one protected place.
              </p>
              <ButtonLink href="/request" className="mt-7" arrow>
            ...
            <div className="space-y-3">
              {[
                [
                  ...
                  Search,
                  "Discover your maker",
                  "Browse verified portfolios, specialities, ratings and real availability.",
                ],
                [
                  "02",
                  MessageCircle,
                  "Shape the brief together",
                  "Share inspiration, negotiate details and accept a clear, versioned quote.",
                ],
                [
                  "03",
                  Ruler,
                  "Share measurements securely",
                  "Use guided templates or book a video call, then approve the exact measurements shared.",
                ],
                [
                  "04",
                  Sparkles,
                  "Watch it come to life",
                  "Receive photo updates through every production stage and preview the concept virtually.",
                ],
                ...
                return (
                  <article
                    ...
                    className="group grid gap-4 rounded-2xl border border-line bg-paper p-6 transition hover:border-wine/35 sm:grid-cols-[52px_1fr_auto] sm:items-center"
                  >
                    <span className="font-display text-3xl text-wine/45">{number as string}</span>
                    <div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2">
          {testimonials.map((item) => (
            ...
            >
              ...
              <blockquote className="mt-5 font-display text-2xl leading-9">
                “{item.quote}”
              </blockquote>
              <figcaption className="mt-6 text-sm font-semibold">
                {item.author}
                <span className="ml-2 font-normal text-muted">{item.location}</span>
              </figcaption>
            </figure>
          ))}
        </div>
      ...
      <section className="container-shell pb-24">
        <div className="relative overflow-hidden rounded-[2rem] bg-[#d8b17e] px-6 py-16 text-center text-ink sm:px-12">
          ...
          <h2 className="relative mx-auto mt-4 max-w-2xl font-display text-5xl leading-none">
            Find the hands that understand your vision.
          </h2>
          <div className="relative mt-7 flex flex-col justify-center gap-3 sm:flex-row">
            <ButtonLink href="/tailors" variant="dark" arrow>
