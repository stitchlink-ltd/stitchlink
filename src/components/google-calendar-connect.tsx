...
export function GoogleCalendarConnect({ connected, googleEmail, syncMilestones }: Props) {
  ...
  async function toggleMilestones(next: boolean) {
    setMilestones(next);
    await fetch("/api/integrations/google-calendar/milestones", {
      method: "POST",
    ...
    <div className="mt-5 rounded-2xl bg-background p-5">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <span
            className={`grid size-10 shrink-0 place-items-center rounded-full ${connected ? "bg-sage/10 text-sage" : "bg-blue/10 text-blue"}`}
          >
        ...
        {connected ? (
          ...
          <a
            ...
            className="rounded-lg bg-wine px-4 py-2 text-xs font-semibold text-white"
          >
            Connect
          </a>
        )}
      ...
      {connected && (
        <label className="mt-4 flex items-center gap-2 text-xs font-semibold">
          <input
            type="checkbox"
            checked={milestones}
            onChange={(event) => toggleMilestones(event.target.checked)}
            className="size-4 "
          />
